...
for superuser in superusers:
    if superuser.username != dotEnvSuperusername or not superuser.check_password(dotEnvSuperuserPassword)  or superuser.email != dotEnvSuperuserEmail:
        superuser.delete()
    else:
        correctSuperuserIsPresent = True

if not correctSuperuserIsPresent:
    User.objects.create_superuser(dotEnvSuperusername, dotEnvSuperuserEmail, dotEnvSuperuserPassword)
