...
class TestUseCaseFiltering(TestCase):
    ...
    def setUp(self):
        ...
        call_command(
            "data_import",
            "use_cases",
            "../../02_work_doc/01_daten/14_use_cases/20240423_daten_use_cases.xlsx",
            ".",
        )
    ...
    def testMultipleFocusElementsSelected(self):
        ...
        # selected types:
        focusElementsForUseCaseObjs = [
            (filteredUseCaseObj.focus.all())
        ...
        for focusStr in selectedFocusElements:
            ...
            for filteredUseCaseObj in filteredUseCaseObjs:
                for currentFocus in filteredUseCaseObj.focus.all():
                    if (
                        focusStr in currentFocus.focus
                        or focusStr in currentFocus.focus_en
                    ):
                        sortUseCasesIntoFocusBuckets[focusStr].append(
                            filteredUseCaseObj
                        )
            ...
            if (
                len(
                    UseCase.objects.filter(
                        degreeOfDetail__icontains=currentChosenElement
                    )
                )
                > 0
            ):
