...
def index(request):
    ...
    explanationText += _("Auf dieser Seite präsentieren wir eine detaillierte Analyse verschiedener Anwendungsfälle im Kontext des Smart Readiness Indicator. Das Ziel ist es die unterschiedlichen Effekte der Digitalisierung aufzudecken und die Interaktion verschiedener Anforderungen aufzuzeigen. Dabei gehen wir auf verschiedene Fragestellungen ein:")
    explanationText +=  "</p>"
    explanationText += "<li>"
    ...
    explanationText += _("Die digitale Anwendung benötigt zahlreiche Messdaten, die Verhaltensmuster erkennen lassen?")
    explanationText += _("Die Anwendung ermöglicht durch Feedback energetische Einsparungen?")
    explanationText += _("Die Auswirkungen digitaler Anwendungen sind durch ihre Wechselwirkungen komplex und vielschichtig.")
    explanationText += _("So werden beispielsweise Ressourcen und Energie für Erstellung und den Betrieb von Messtechnik benötigt, ")
    explanationText += _("eine genauere Messung ermöglicht aber auch ein individuelleres Feedback an Nutzende oder ein genaues Erkennen von Fehlern.")
    explanationText += _("Hier sollen einige dieser wechselwirkenden Effekte gesammelt und in ihren Auswirkungen begreifbar gemacht werden.")
    explanationText += "</p><p>"
    explanationText += _("Als Basis wird das Rahmenwerk des Smart Readiness Indicator genutzt, der zur Bewertung der Intelligenzfähigkeit von Gebäuden nach Vorschriften der EU entwickelt wurde.")
    explanationText += _("In diesem Rahmenwerk sind „Services“ definiert, welche hier als Use-Cases betrachtet werden.")
    explanationText += _("Dabei werden die folgenden Aspekte betrachtet:")
    explanationText += "</p><ul><li>"
    explanationText += _("Use-Case: Beschreibt den konkreten Service, auf den sich bezogen wird und wodurch die Effekte/Wirkungen hervorgerufen werden.")
    explanationText += "</li><li>"
    explanationText += _("Wirkebene: Die Ebenen, auf die sich der Use-Case bzw. die Aktion/Durchführung bezieht.")
    explanationText += "</li><li>"
    explanationText += _("Level: Die Abstufungen der Durchführung des Use-Cases.")
    ...
    explanationText += _("Reporting information regarding electricity consumption")
    explanationText += "</td><td>"
    explanationText += _("None")
    explanationText += "</td><td>"
    explanationText += _("Reporting on current electricity consumption on building level")
    explanationText += "</td><td>"
    explanationText += _("Real-time feedback or benchmarking on building level")
    explanationText += "</td><td>"
    explanationText += _("Real-time feedback or benchmarking on appliance level")
    explanationText += "</td><td>"
    explanationText += _("Real-time feedback or benchmarking on appliance level with automated personalized recommendation")
    explanationText += "</td></tr></table><ul><li>"
    explanationText += _("Perspektive: Die Bereiche, auf denen sich die Durchführung des Use-Cases auswirkt, beispielsweise ökonomisch, ökologisch, aus der Nutzendenperspektive oder der technischen Sichtweite.")
    explanationText += "</li><li>"
    explanationText += _("Effekt: Die mit der Durchführung des Use-Cases einhergehenden Wirkungen bzw. erzielten/intendierten Effekte.")
    explanationText += "</li></ul><p>"
    explanationText += _("Eine detaillierte Betrachtung anhand von Zeitreihen findet anhand des Services „Reporting information regarding electricity consumption“ statt.")
    explanationText += "<br>"
    explanationText += _("Um diese Effekte detaillierter darzustellen, wurde der IDEAL-Datensatz genutzt. In diesem Datensatz sind Monitoringdaten für 255 Haushalte aus den UK und Schottland enthalten.")
    explanationText += _("39 dieser Haushalte sind in einem engmaschigen System gemonitort worden.")
    explanationText += _("Für die Darstellung sind die engmaschig gemonitorten Haushalte 62, 105 und 106 genutzt worden.")
    explanationText += _("Trotz dieses engmaschigen Monitorings ist ersichtlich, dass nicht immer alle Sensoren funktionieren (Haushalt 106).")
    explanationText += "<br>"
    explanationText += _("Dargestellt sind der durchschnittliche Verbrauch in einem Zeitfenster von fünf Sekunden, 15 Minuten und einer Stunde.")
    explanationText += _("Dabei sind sowohl die einzelnen Werte der Haushalte dargestellt als auch die Summe der Lastgänge (three buildings).")
    explanationText += _("Die beispielhafte Darstellung zeigt, wie sich verschiedene Aggregationen (zeitlich und räumlich) auswirken.")
    explanationText += _("Mittels hochfrequenter Messungen sind einzelne Effekte sehr gut zu sehen, welche in anderen Aggregationen untergehen.")
    explanationText += _("Damit sollen die Nutzenden ein quantitatives und qualitatives Gefühl für die Effekte der Digitalisierung und deren Wechselwirkungen bekommen.")
    explanationText += _("So werden beispielsweise Ressourcen und Energie für Erstellung und den Betrieb von Messtechnik benötigt, ")
    explanationText += _("eine genauere Messung ermöglicht aber auch ein individuelleres Feedback an Nutzende oder ein genaues Erkennen von Fehlern.")
    explanationText += "<br>"
    explanationText += _("Das Ziel ist es, die unterschiedlichen Effekte der Digitalisierung aufzudecken und die Interaktion verschiedener Anforderungen aufzuzeigen.")
    explanationText += _("Dabei gehen wir auf verschiedene Fragestellungen ein:")
    explanationText += "</p><ul><li>"
    explanationText += _("Wie wirkt sich die Frequenz (5s, 15-minütig, 1-stündig) auf die Sichtbarkeit von Effekten aus?")
    explanationText += "</li><li>"
    explanationText += _("Wie wirken sich einzelne Aggregationen (Anlagen, Gesamtverbrauch, drei Gebäude) auf die Sichtbarkeit von Effekten aus?")
    explanationText += "</li><li>"
    explanationText += _("Was ist in dem Graph zu sehen?")
    explanationText += "</li></ul><p>"
    ...
    explanationText += _("Pullinger et al., The IDEAL household energy dataset, electricity, gas, contextual sensor data and survey data for 255 UK homes, ci Data 2021 May 28;8(1):146. doi: 10.1038/s41597-021-00921-y.")
