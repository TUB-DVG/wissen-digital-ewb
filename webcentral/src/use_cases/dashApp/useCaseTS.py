            ...
            [
                ...
                dcc.Dropdown(
                    ...
                    options=[
                        ...
                        {
                            "label": "Gesamte Wohnungen",
                            "value": "full_building",
                        },
                        {
                            "label": "Aggregierte Wohnungen",
                            "value": "aggregated_buildings",
                        },
        ...
        for i in intervals:
            ...
            if scale == "all" or scale == "equipment":
                ...
                if any(
                    ...
                    for col in [
                        ...
                        if re.match(r"Kühl-_und_Gefrierschrank.*", col)
                    ]:
                        fig.add_trace(
                            go.Scatter(
                                x=data.index,
                ...
                if any(re.match(r"Durchlauferhitzer.*", col) for col in data.columns):
                    for col in [
                        col
                        for col in data.columns
                        if re.match(r"Durchlauferhitzer.*", col)
                    ]:
                        fig.add_trace(
                            go.Scatter(
                                x=data.index,
            ...
            if scale == "all" or scale == "full_building":
                if any(
                    ...
                    for col in [
                        col
                        for col in data.columns
                        if re.match(r"Hauptanschluss.*", col)
                    ]:
                        fig.add_trace(
                            go.Scatter(
                                x=data.index,
                                y=data[col],
                                mode="lines",
                                name=f"{b} {i} Gesamter Stromverbrauch",
                            )
                        )
            ...
            if scale == "all" or scale == "aggregated_buildings":
                        ...
                        for col in [
                            ...
                            fig.add_trace(
                                go.Scatter(
                                    ...
                                    y=data[col],
                                    mode="lines",
                                    name=f"{b} {i} Gesamtstromverbrauch ",
                                )
                            )
                ...
                fig.add_trace(
                    go.Scatter(
                        x=aggregated_data.index,
                        y=aggregated_data["total"],
                        mode="lines",
                        name=f"{b} {i} Aggregierter Gesamterverbrauch",
                    )
                )
