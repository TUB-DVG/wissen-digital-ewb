...
class Component(models.Model):
    ...
    powerUseCasePhaseActiveSuperscript = models.CharField(
    ...
    globalWarmingPotentialProdSup = models.CharField(
        max_length=100, blank=True, null=True
    )

    globalWarmingPotentialUsePhase = models.FloatField(blank=True, null=True)
    globalWarmingPotentialUsePhaseSup = models.CharField(
        max_length=100, blank=True, null=True
    )
    ...
    def energyConsumptionUsePhaseActiveRoundedSup(self):
        ...
        if self.energyConsumptionUsePhaseActive is not None:
            roundedNumber = round(
                self.energyConsumptionUsePhaseActive, decimalPosToRound
            )
            if self.powerUseCasePhaseActiveSuperscript is not None:
                return Template(
                    str(roundedNumber)
                    + f"<sup class='supForNumValues'>{self.powerUseCasePhaseActiveSuperscript}</sup>"
                ).render(Context({}))
            else:
    ...
    def globalWarmingPotentialProductionRoundedSub(self):
        ...
        if self.globalWarmingPotentialProduction is not None:
            roundedNumber = round(
                self.globalWarmingPotentialProduction, decimalPosToRound
            )
            if self.globalWarmingPotentialProdSup is not None:
                return Template(
                    str(roundedNumber)
                    + f"<sup class='supForNumValues'>{self.globalWarmingPotentialProdSup}</sup>"
                ).render(Context({}))
    ...
    def globalWarmingPotentialUsePhaseRoundedSub(self):
        ...
        if self.globalWarmingPotentialUsePhase is not None:
            roundedNumber = round(
                self.globalWarmingPotentialUsePhase, decimalPosToRound
            )
            if self.globalWarmingPotentialUsePhaseSup is not None:
                return Template(
                    str(roundedNumber)
                    + f"<sup class='supForNumValues'>{self.globalWarmingPotentialUsePhaseSup}</sup>"
                ).render(Context({}))
