...
class TestRounding(TestCase):
    ...
    def testFindLastDecimalPlaces(self):
        ...
        returnValueFloatThree = componentObj._findLastDecimalPlaces(
            str(exampleFloatThree)
        )
        self.assertEqual(returnValueFloatThree, 2)

        exampleFloatFour = 10
        returnValueFloatFour = componentObj._findLastDecimalPlaces(
            str(exampleFloatFour)
        )
...
class TestDataImport(TestCase):
    ...
    def testImportWholeExcel(self):
        ...
        call_command(
            "data_import",
            "component_list",
            "../../02_work_doc/01_daten/12_component_list/componentList_oneSheet.xlsx",
            ".",
        )

        importedComponents = Component.objects.all()
        self.assertGreaterEqual(
            len(importedComponents),
            44,
            "After import of the production-data file, there should be at least 44 Components in the database.",
        )
            ...
            self.assertGreaterEqual(
                len(
                    Component.objects.filter(
                        description_en=row[header.index("Beschreibung__en")]
                    )
                ),
                1,
            )
