...
class checkDifferencesInDatabase(TransactionTestCase):
    ...
    def testIfYAMLRepresentsDBState(self) -> None:
        ...
        fileNameContainingTestData = (
            "../../02_work_doc/10_test/04_testData/enargus_testDatabaseFile.csv"
        )
        management.call_command(
            "data_import",
            fileNameContainingTestData,
        ...
        for databaseDiffObj in listOfDBDifferenceObjs:
            ...
            self._checkIfDifferencesFromYAMLAreInDB(
                nestedDictContainingDiffs, dictMappingToTable
            )
    ...
    def testExecuteDatabaseChangeskeepCurrent(self):
        ...
        loadInitialStateOfDB = (
            "../../02_work_doc/10_test/04_testData/enargus_load10Datasets.csv"
        )
        management.call_command(
            "data_import",
            loadInitialStateOfDB,
    ...
    def testLoadSimpleToolData(self):
        ...
        readToolTestfile = pd.read_excel(
            "../../02_work_doc/10_test/04_testData/test_data_tool.xlsx"
        )
        ...
        for key in toolAsDict.keys():
            if key != "name":
                ...
                if (
                    getattr(acceptMissionToolInDB, key).__class__.__name__
                    == "ManyRelatedManager"
                ):
                    ...
                    for index, value in enumerate(manyToManyValues):
                        if index == 0:
                            manyToManyString += value.__getattribute__(
                                key[0].lower() + key[1:]
                            )
                        else:
                            manyToManyString += ", " + value.__getattribute__(
                    ...
                    if getattr(acceptMissionToolInDB, key) is None:
                        ...
                        self.assertEqual(
                            getattr(acceptMissionToolInDB, key),
                            toolAsDict[key][0],
                        )
    ...
    def testToolCheckLastUpdateEmpty(self):
        ...
        self.assertEqual(
            Tools.objects.filter(name="lastUpdateTest1")[0].lastUpdate,
            "unbekannt",
        )
    ...
    def testLoadSimplePublicationData(self):
        ...
        readPublicationTestfile = pd.read_excel(
            "../../02_work_doc/10_test/04_testData/test_data_publications.xlsx"
        )
        ...
        for keyInFileDict in publicationAsDict["title"].keys():
            publicationInDB = list(
                Publication.objects.filter(
                    title=publicationAsDict["title"][keyInFileDict]
                )
            )[-1]
            ...
            for key in publicationAsDict.keys():
                # check if attribute is a ManyToMany-relation:
                if (
                    getattr(publicationInDB, key).__class__.__name__
                    == "ManyRelatedManager"
                ):
                    ...
                    self.assertEqual(
                        manyToManyString, publicationAsDict[key][keyInFileDict]
                    )
                else:
    ...
    def testLoadingAndUpdatingTags(self) -> None:
        ...
        simpleTagsModifiedDataset = "../../02_work_doc/10_test/04_testData/schlagwoerter_simpleTestDataModified.csv"
        if newestYAMLFile == "" or self._checkIfFileIsOlderThan2Minutes(
            newestYAMLFilePath
        ):
            management.call_command(
                "data_import",
    ...
    def _doAssertation(
        ...
        if not self._checkIfIDisNone(stateDict):
            if not (
                ...
                if (
                    str(currentTableModel)
                    == "<class 'schlagwoerter.models.KeywordRegisterFirstReview'>"
                ):
                    schlagwortregisterQuery = currentTableModel.objects.filter(
                        **stateDict
                    )
                    ...
                    str(currentTableModel)
                    == "<class 'project_listing.models.ModuleAssignment'>"
                ):
                ...
                for numberPosition in range(1, 7):
                    ...
                    for query in queryForCurrTag:
                        if query.schlagwortregister_id == int(
                            schlagwortregisterIDcurrent
                        ):
                            self.assertTrue(False)
    ...
    def _getNewestYAML(self) -> str:
        ...
        for currentFilename in fileContents:
            if ".yaml" in currentFilename:
                fileTimestamp = os.path.getmtime(
                    os.path.join("tests/data/", currentFilename)
                )
        ...
        with open(filename, "r") as file:
            for element in yaml.load_all(file, Loader=yaml.Loader):
                ...
                if onlyLoadObjWithSetKeepAttr:
                    if (
                        element.keepCurrentState is not None
