...
def change_values(df):
    ...
    for col in df.columns:
        df[col] = df[col].replace({'nicht gewählt': np.nan, 'ausgewählt': col.split(': ')[-1]})
        
    
    # change certain values
    df.replace(to_replace=r'(?i).?phyton.?', value='Python', inplace=True, regex=True)
    df.replace(to_replace=r'(?i).?matlab.?', value='MATLAB', inplace=True, regex=True)
    df.replace(to_replace=r'(?i).?mondas.?', value='Mondas', inplace=True, regex=True)
    df.replace(to_replace=r'(?i).?nicht zutreffend.?', value=np.nan, inplace=True, regex=True)
    #df.replace(to_replace=r'(?i).?python.?', value='Python', inplace=True, regex=True)
    df.replace(to_replace=r'(?i).?speziell.*kirchen.*', value='Software für Kirchenbauten', inplace=True, regex=True)
    return df
...
def read_and_process_excel(file_path):
    ...
    for col in df_temp.columns:
        ...
        # rename conlums
    df_temp.columns = [col.split(':')[0] if ':' in col else col for col in df_temp.columns]

    # delete the columns end with 'Anzahl', 'Eigene', 'Sonstige', or 'Entwicklung'
    df.drop(columns=df.filter(regex=r'Anzahl|(Eigene|Sonstige|Entwicklung)$').columns, inplace=True)
...
def data_relation_process(df):
    ...
    for col in unique_columns:
        try:
            df_relation[col] = df_temp[col].agg(lambda row: ';'.join(filter(None, row.dropna().astype(str).unique())),axis=1)
        except:
...
def add_br_tag_with_ellipsis(s, max_length=20):
    ...
    if len(s) <= max_length:
        ...
        if space_pos1 == -1:
            ...
            return s[:space_pos1] + ' <br>' + s[space_pos1+1:max_length*2] + '...'
        
        else:
            return s[:space_pos1] + ' <br>' + s[space_pos1+1:space_pos2] + '...'

def create_sankey(df_relation, df_anzahl, opacity, max_length, colorred, combiTimes=False):
    ...
    for _, row in df_relation.iterrows():
        ...
        for i, col in enumerate(df_relation.columns[1:]):  # jump first col
            current_node = row[col] if row[col] not in [None, ""] else "Keine Antwort ausgewählt"
            ...
            # count node_weights
            node_weights[node_key] = node_weights.get(node_key, 0) + 1/(row.iloc[0]**combiTimes)  # adjust 
            
            if prev_node is not None:
                link_key = (prev_node, node_key)
                link_counts[link_key] = link_counts.get(link_key, 0) + 1/(row.iloc[0]**combiTimes)  # adjust 
    ...
    color_palette = [f'rgba({np.random.randint(0, 150)}, {np.random.randint(50, 255)}, {np.random.randint(50, 255)}, 0.8)' for _ in unique_names]
    ...
    for name, pos in zip(column_names, column_positions):
        annotations.append(
            dict(
                ...
                text= add_br_tag_with_ellipsis(f'{name}: ', 30)  +'<br>' + f'{df_anzahl.loc[0,name]} Antworten',
...
app.layout = html.Div([
    ...
    dcc.Slider(id='color-of-none-slider', min=0, max=255, value=255, step=10),
...
def process_and_visualize_excel(n_clicks, opacity, combi, maxlength, color_of_none):
    ...
    fig =  create_sankey(df_relation, df_anzahl, opacity, maxlength, color_of_none, combiTimes=combi)
