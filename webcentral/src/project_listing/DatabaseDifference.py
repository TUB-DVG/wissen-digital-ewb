...
class DatabaseDifference(yaml.YAMLObject):
    ...
    def checkIfUserInputIsValid(self) -> bool:
        ...
        if self.keepCurrentState is None or self.keepPendingState is None:
            ...
            if (
                bool(self.keepCurrentState) == True
                and bool(self.keepPendingState) == False
            ):
                return True
            elif (
                bool(self.keepCurrentState) == False
    ...
    def _findModelNameForKey(self, allModels, curentTableFromKey):
        """ """
        currNameWithUpperLetter = (
            curentTableFromKey[0].upper() + curentTableFromKey[1:]
        )
    ...
    def checkIfConflictIsConsistentWithDatabase(self):
        ...
        try:
            classNameOfTable = getattr(
                allModels.__getattribute__(rootTableName.split(".")[0]),
                rootTableName.split(".")[1],
            ).field.related_model
        except:
            classNameOfTable = allModelsTools.__getattribute__(
                rootTableName.split(".")[0]
            )
        ...
        for rootTableFieldName in dictofRootTable:
            ...
            if "_id" in rootTableFieldName or rootTableFieldName == "id":
                ...
                currentStateInRootTable = globals()[
                    parentTableName
                ].objects.filter(
                    **{
                ...
                querySetForPendingObj = classNameOfTable.objects.filter(
                    **{rootTableFieldName: idOfConflictingPendingObj}
                )
                querySetForCurrentObj = globals()[
                    parentTableName
                ].objects.filter(
                    **self.identifer,
                )
                ...
                if (
                    len(querySetForPendingObj) > 0
                    and (
                        len(querySetForCurrentObj) > 0
                        or idOfConflictingCurrentObj is None
                    )
                    and len(currentStateInRootTable) > 0
                ):
                    try:
                        nameOfFieldRelatesToTable = (
                            self.findFieldNameRelatingToForeignTable(
                                globals()[parentTableName], classNameOfTable
                            )
                        )
                    except:
                        nameOfFieldRelatesToTable = None
                    try:
                        currentStateRowObj = currentStateInRootTable[
                            0
                        ].__getattribute__(
    ...
    def postprocessAfterReadIn(self) -> None:
        ...
        for tableNameKey in self.differencesSortedByTable:
            ...
            for diffAttribute in list(
                self.differencesSortedByTable[tableNameKey][
                    "currentState"
                ].keys()
            ):
                if isinstance(
