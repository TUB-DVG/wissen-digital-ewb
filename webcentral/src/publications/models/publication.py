...
class Publication(models.Model):
    ...
    def _produce_author_lists(self):
        ...
        self.title_ends_with_punct = (
            self.title[-1] in [".", "!", "?"] if len(self.title) > 0 else False
        )
        ...
        if len(self.authors_list) > 2:
            self.authors = ", and ".join(
                [", ".join(self.authors_list[:-1]), self.authors_list[-1]]
            )
    ...
    def keywords_escaped(self):
        return [
            (keyword.strip(), quote_plus(keyword.strip()))
    ...
    def key(self):
        ...
        publications = Publication.objects.filter(
            year=self.year, authors__icontains=author_lastname
        ).order_by("month", "id")
