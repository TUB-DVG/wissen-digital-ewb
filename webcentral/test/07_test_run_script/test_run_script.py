...
class TestRunScript(TestCase):
    ...
    def testPreEnargus(self):
        ...
        self.assertEqual(
            result.stdout,
            "pre_enargus: Execute the preprocessing step to convert xml-enargus data to csv-data\\n./run pre_enargus <source-xml-file> <target-csv-file>\n",
        )
        ...
        try:
            result = run(
                [
                    "./run",
                    "pre_enargus",
                    "webcentral/doc/01_data/01_pre_pro/20240610_Verteiler_EWB_Projekte.xlsx",
                    "webcentral/doc/01_data/01_pre_pro/modulzuordnung_test.csv",
                ],
                stdout=PIPE,
                stderr=PIPE,
                text=True,
        ...
        self.assertTrue(
            "enargus_test.csv" in listdir("webcentral/doc/01_data/01_pre_pro")
        )
    ...
    def testPreModul(self):
        ...
        self.assertEqual(
            result.stdout,
            "pre_modul: Execute the preprocessing step to convert xlsx-modul data to csv-data\\n./run pre_modul <source-xlsx-file> <target-csv-file>\n",
        )
        try:
            result = run(
                [
